package org.vertex.bukkit.scheduler;
        ...
        try {
            if (every == 0) {
                if (startAfter == 0) {
                    ...
                    args = new Object[]{BukkitPluginContainer.getCurrentPlugin(), runnable};
                } else {
                    ...
                    args = new Object[]{BukkitPluginContainer.getCurrentPlugin(), runnable, startAfter};
                }
            } else {
                if(times == 0) {
                    ...
                    args = new Object[]{BukkitPluginContainer.getCurrentPlugin(), runnable, startAfter, every};
                } else {
                    BukkitRunnable task = new TaskExecutor(times, runnable);
                    if(async) {
                        return task.runTaskTimerAsynchronously(BukkitPluginContainer.getCurrentPlugin(), startAfter, every);
                    } else {
                        return task.runTaskTimer(BukkitPluginContainer.getCurrentPlugin(), startAfter, every);
                    }
                }
            }
